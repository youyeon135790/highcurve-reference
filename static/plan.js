...
(function () {
  ...
  function renderPlanTab(plan, p) {
    ...
    const left = `<aside class="plan-side"><div class="title-card"><div class="ph-kicker">내 릴스 기획안</div><h2>${esc(plan.title)}</h2>${plan.thumbnail_text ? `<span class="ttag">썸네일: ${esc(plan.thumbnail_text)}</span>` : ""}${plan.one_line ? `<p>${esc(plan.one_line)}</p>` : ""}</div>
      ...
      ${plan.script_full ? `<div class="side-sec"><h3>🎙 전체 대본 <small>한 호흡으로 · 프롬프터용</small></h3>${ed("script_full", plan.script_full, "capbox")}<button class="btn small" style="margin-top:6px" onclick="navigator.clipboard.writeText(CUR.plan.script_full);toast('대본 복사됨')">복사</button></div>` : ""}
