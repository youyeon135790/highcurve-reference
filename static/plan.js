...
(function () {
  ...
  const save = () => { try { localStorage.setItem(WKEY(), JSON.stringify({ ...W, refsCache: W.refsCache.slice(0, 48), topics: W.topics.slice(0, 32) })); } catch (e) {} };
  ...
  async function renderRefsStep() {
    const sel = W.refs;
    const mainL = W.refsCache.filter(x => !x.fill), fillL = W.refsCache.filter(x => x.fill);
    ...
    <div class="wiz-sub" style="margin-top:18px">또는 아래에서 고르세요 <small class="muted">내 키워드로 찾은 터진 릴스예요</small></div>${kwBoxHtml()}
    <div class="row" style="gap:8px;flex-wrap:wrap;margin:8px 0">${STATIC ? "" : `<button class="btn" onclick="wizCrossRefs()" title="업종은 달라도 구조가 좋으면 대입해 봅니다">다른 업종에서 터진 구조 보기</button>`}${W.refsAi && (W.refsAiOrder || []).length ? `<button class="btn" onclick="wizRefsAiSort()">AI 추천순으로 다시 정렬</button>` : ""}</div>
    ${refSelHtml()}
  ...
  function refSelHtml() { const sel = W.refs || [];
    return `<div class="wiz-sel">${sel.length ? `<b>고른 릴스 ${sel.length}/3</b>` + sel.map(id => `<span class="selchip ${W.mainRef === id ? "is-main" : ""}"><button onclick="wizMain(${id})" title="주 레퍼런스로 지정">${W.mainRef === id ? "★ 주 레퍼런스" : "☆ 주로 바꾸기"}</button><i>@${esc((W.refsCache.find(x => x.id === id) || {}).account || id)}</i><button onclick="wizPick(${id})" aria-label="빼기">✕</button></span>`).join("") : "<b>아직 고른 릴스가 없어요</b><span class=\"muted\">아래에서 눌러 고르세요</span>"}</div>`; }
  function refreshRefs(ids) { ids.forEach(id => { const el = document.querySelector(`.refcard[data-id="${id}"]`); const x = W.refsCache.find(y => y.id === id); if (el && x) el.outerHTML = refCard(x, W.refs.includes(id)); }); const sel = document.querySelector(".wiz-sel"); if (sel) sel.outerHTML = refSelHtml(); }
  ...
  window.wizKwSearch = () => { if (!(W.kwOn || []).length) return toast("키워드를 하나 이상 켜 주세요"); W.keyword = W.kwOn[0]; W.selSubs = W.kwOn.slice(1); W.refsCache = []; W.refsAi = false; W.refsAiOrder = []; save(); renderWizard(); wizSearchRefs(); };
  ...
  function renderTopicStep() {
    ...
    <details class="wiz-more tune" ${moreOpen} ontoggle="wizDev(this.open)"><summary><b>그 밖의 디벨롭 설정</b><span>영상 길이 · 출연자 캐릭터 · 영상 끝에서 보낼 곳 — 비워두면 고른 릴스를 그대로 따릅니다</span><i>열기</i></summary>
  ...
  let EZSEC = (() => { try { const v = localStorage.getItem("ezsec3"); return EZ.some(x => x[0] === v) ? v : "all"; } catch (e) { return "all"; } })();
  window.ezSec = (k) => { EZSEC = k; try { localStorage.setItem("ezsec3", k); } catch (e) {} renderPlan2(CUR); const el = document.querySelector(".pv-tabs"); if (el && el.getBoundingClientRect().top < 0) window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 12 }); };
    ...
    return `<section class="pv-card"><div class="pv-h"><h2>요약</h2><span>이것만 봐도 영상이 그려지게</span></div><table class="pv-kv">${sumRows}</table></section>
      <section class="pv-card"><div class="pv-h"><h2>첫 문장 고르기</h2><span>'이걸로'를 누르면 1번 장면과 썸네일 문구가 같이 바뀝니다</span></div>${hookTable}
        <div class="hk-change"><div class="hk-change-h"><b>훅 바꾸기</b><span>유형을 누르면 그 유형의 준희즘 템플릿으로 첫 문장 3개를 새로 뽑아 위 표에 추가해요 (10초 안팎${admin ? "" : " · 기획안당 5번"})</span></div>
          <div class="hk-types">${types.map(t => `<button class="${HOOKBUSY === t ? "busy" : ""}" ${HOOKBUSY ? "disabled" : ""} onclick="hookConvert('${esc(p.id)}', ${JSON.stringify(t).replace(/"/g, "&quot;")})">${HOOKBUSY === t ? "뽑는 중…" : esc(t)}</button>`).join("")}</div>
  ...
  function ezScript(plan, p) {
    ...
    const rows = sc.map((x, si) => { const part = partOf(x); const firstOfPart = !seen.has(part); seen.add(part);
      ...
      return `<tr class="p-${esc(part)}"><td class="c-no"><b>${esc(x.no)}</b><span>${esc(secTxt(x.sec))}초</span></td><td class="c-sk">${skCell(x)}</td><td class="c-part"><b>${esc(part)}</b><span>${esc(stage)}</span></td><td class="c-say"><div>${hasPh ? phHtml(x.say) : ed("B_plan.scenes." + si + ".say", x.say, "inline", "span")}</div>${cLine("자막", phHtml(x.caption || ""))}</td><td class="c-sub">${phHtml(x.screen || "")}${cLine("장치", esc(hold))}${cLine("편집", esc(x.edit || ""))}</td></tr>`; }).join("");
    return `<section class="pv-card"><div class="pv-h"><h2>대본</h2><span>${esc(plan.length_sec)}초 · ${sc.length}장면 · 대사를 누르면 바로 고칠 수 있고, 그림을 누르면 찍는 법이 크게 보여요</span><div class="pv-h-right"><button class="pv-btn" onclick="copyScript()">대본 복사</button><button class="pv-btn" onclick="openPrompter()">크게 보기</button><button class="pv-btn" onclick="humanize('${esc(p.id)}')">내 말투로 다듬기</button><button class="pv-btn" onclick="reSketch('${esc(p.id)}')">장면 그림 다시 그리기</button>${document.body.classList.contains("admin") ? `<button class="pv-btn" onclick="likeScript('${esc(p.id)}')">좋은 대본으로 저장</button>` : ""}</div></div>
  ...
  function planListHtml(plans) {
    ...
    const shown = SHOWN = plans.filter(p => (PF.folder === "전체" || (PF.folder === "(폴더 없음)" ? !p.folder : p.folder === PF.folder)) && (!PF.stage || (p.stage || "기획 중") === PF.stage));
    ...
    return `<div class="plan-toolbar" style="margin-bottom:10px"><h2 style="margin:0">내 기획 <small>${plans.length}개</small></h2><div class="row" style="gap:6px;flex-wrap:wrap"><select class="chip" onchange="PF_set('stage', this.value)"><option value="">진행 단계: 전체</option>${STAGES.map(s => `<option ${PF.stage === s ? "selected" : ""}>${s}</option>`).join("")}</select><button class="btn small" onclick="newFolder()">+ 새 폴더</button><button class="btn small ${PSEL ? "p" : ""}" onclick="planSelMode()">${PSEL ? "정리 끝내기" : "여러 개 정리하기"}</button><a class="btn small" href="#/plan/box">내 저장소</a><a class="btn small" href="#/plan/trash">휴지통${TRASH_N ? " " + TRASH_N : ""}</a>${admin ? `<button class="btn small" onclick="PF_set('all', ${PF.all ? "false" : "true"})">${PF.all ? "내 기획안만 보기" : "모든 사용자 기획안 보기"}</button>` : ""}</div></div>
      ...
      ${shown.length ? `<div class="plangrid">${shown.map(p => { const on = SEL.has(p.id); const open = PSEL ? `planSelToggle('${p.id}')` : `location.hash='#/plan/${p.id}'`; return `<div class="plancard ${PSEL ? "selmode" : ""} ${on ? "sel" : ""}"><div class="pc-cover" onclick="${open}">${PSEL ? `<span class="pc-check">${on ? "✓" : ""}</span>` : ""}${p.cover ? `<img src="${esc(p.cover)}" onerror="imgRetry(this)">` : ""}<span class="pc-state ${p.status === "done" ? "ok" : ""}">${p.status === "done" ? "완성" : "프롬프트만"}</span></div><div class="pc-body"><b onclick="${open}" style="cursor:pointer">${esc(p.title || "(제목 없음)")}</b><div class="muted">${p.set_id ? "세트 · " : ""}${esc(p.job || "")} · ${esc(p.created_at || "")}${PF.all && p.user_id ? ` · 사용자 ${p.user_id}` : ""}</div>
        ${PSEL ? "" : `<div class="pc-meta"><select class="chip stage-${STAGES.indexOf(p.stage || "기획 중")}" onchange="setPlanMeta('${p.id}', {stage: this.value})">${STAGES.map(s => `<option ${(p.stage || "기획 중") === s ? "selected" : ""}>${s}</option>`).join("")}</select><select class="chip" onchange="if(this.value==='__new'){newFolder('${p.id}')}else{setPlanMeta('${p.id}', {folder: this.value})}"><option value="" ${!p.folder ? "selected" : ""}>폴더 없음</option>${folders.map(f => `<option ${p.folder === f ? "selected" : ""}>${esc(f)}</option>`).join("")}<option value="__new">+ 새 폴더로…</option></select><button class="pc-trash" onclick="cardTrash('${p.id}')" title="휴지통으로 옮기기">휴지통</button></div>`}</div></div>`; }).join("")}</div>` : `<div class="empty"><b>${plans.length ? "이 조건에 맞는 기획이 없어요" : "아직 만든 기획이 없어요"}</b>${plans.length ? "" : "새 기획 만들기로 첫 기획안을 만들어보세요."}</div>`}`;
  }
  ...
  window.viewPlanSet = async function (sid) {
    ...
    const draw = async () => {
      ...
      if (all && wasDone === false && lastV && lastV.status === "done") { makingDone(sid); location.replace(location.pathname + location.search + "#/plan/" + lastV.id); return true; }   // 방금 만든 편이 다 되면 바로 그 기획안으로
      wasDone = all;
