...
(function () {
  ...
  function renderPlanTab(plan, p) {
    ...
    const left = `<aside class="plan-side"><div class="title-card"><div class="ph-kicker">내 릴스 기획안</div><h2>${esc(plan.title)}</h2>${plan.thumbnail_text ? `<span class="ttag">썸네일 문구: ${esc(plan.thumbnail_text)}</span>` : ""}${plan.one_line ? `<p>${esc(plan.one_line)}</p>` : ""}<div class="tc-meta">${esc(plan.length_sec)}초 · ${scenesAll.length}씬 · ${esc((p.brief || {}).job || "")}</div></div>
      ...
      ${plan.script_full ? `<div class="side-sec"><h3>② 전체 대본 <small>한 호흡으로 읽기 · 프롬프터용</small></h3>${ed("script_full", plan.script_full, "capbox script")}<button class="btn small" style="margin-top:6px" onclick="navigator.clipboard.writeText(CUR.plan.script_full);toast('대본 복사됨')">복사</button></div>` : ""}
    ...
    const scene = (s) => { const f = s.framing || {}; const g = s.guide || {}; const si = scenesAll.indexOf(s); const simple = VIEW === "simple";
      return `<div class="scene v-${VIEW}"><div class="scene-head"><span class="scene-no">${s.no}</span><span class="scene-sec">${esc(s.sec)}초</span><span class="tag part-${esc(s.part || "")}">${esc(PART[s.part] || s.part || "")}</span>${f.shot ? `<span class="muted">${esc(f.shot)}${f.camera ? " · " + esc(f.camera) : ""}</span>` : ""}</div>
        <div class="scene-grid"><div class="scene-visual">${s.sketch ? `<figure class="sb-ref sk-img"><img src="${esc(s.sketch)}" loading="lazy" title="${esc(s.sketch_prompt || "")}"><figcaption>구도 <a href="#" onclick="event.preventDefault();reSketch('${esc(p.id)}', ${s.no})">다시</a></figcaption></figure>` : sketch(f)}${refFrameImg(p, s.ref_frame)}</div>
        <div class="scene-body"><div class="say-big">${ed("B_plan.scenes." + si + ".say", s.say, "say")}</div><div class="cap-line">💬 자막: ${ed("B_plan.scenes." + si + ".caption", s.caption, "inline", "span")}${f.text_pos ? ` <span class="muted">(${esc(f.text_pos)})</span>` : ""}</div>
          ...
          ${simple ? `<details class="scene-more"><summary>촬영 가이드 · 캡컷 편집 보기</summary>` : ""}
          <div class="guidebox"><b>촬영 가이드</b><div><b>구도</b> ${esc(g.composition || [f.shot, f.camera].filter(Boolean).join(" · ") || "-")}</div>${g.light ? `<div><b>조명</b> ${esc(g.light)}</div>` : ""}${g.props ? `<div><b>소품</b> ${esc(g.props)}</div>` : ""}${g.action ? `<div><b>행동</b> ${esc(g.action)}</div>` : ""}</div>
    ...
    return `<div class="plan-layout">${left}<div class="plan-main"><div class="plan-toolbar"><h2 style="margin:0">🎬 씬별 대본 <small>${scenesAll.length}씬 · ${esc(plan.length_sec)}초 · 눌러서 바로 고치기</small></h2><div class="seg">${[["simple", "간단히"], ["detail", "자세히"], ["table", "표"]].map(([k, v]) => `<button class="${VIEW === k ? "on" : ""}" onclick="planView('${k}')">${v}</button>`).join("")}</div></div>
  ...
  function renderRefsTab(p) {
    ...
    return `<div class="row" style="justify-content:flex-end;margin-bottom:8px"><button class="btn small" onclick="refTable()">${REF_TABLE ? "카드로 보기" : "표로 보기"}</button></div>` + refs.map((r, i) => {
      ...
      const rowsAuto = (an.timeline || []).map(x => ({ t: `${x.t0}~${x.t1}`, frames: x.frames || [], screen: "", script: x.script, caption: (x.captions || []).map(c => c.text).join(" / "), text_style: (x.captions || [])[0] ? `${x.captions[0].pos} · ${x.captions[0].size}` : "", transition: x.transition, sfx: (x.sfx || []).map(e => e.t + "s").join(", "), effect: "", point: x.capcut }));
      ...
      return `<section class="refsec">
        <div class="ref-head"><img src="${esc(r.thumbnail || "")}" onerror="this.remove()"><div><div class="ph-kicker">레퍼런스 ${i + 1}</div><div class="ref-name">@${esc(r.account)} <a href="${esc(r.url)}" target="_blank" class="btn small">원본 ↗</a></div><div class="ref-metrics"><span>${esc(r.industry || "")}</span><span>▶ ${fmt(r.views)}</span><span>❤ ${fmt(r.likes)}</span><span>${esc(r.posted_at || "")}</span></div><div class="ref-desc">${esc(r.description || "")}</div></div></div>
  ...
  function renderProTab(plan, p) {
    ...
    return `<div class="brief-top"><div><div class="ph-kicker">편집 외주서</div><div class="brief-summary">${esc(eb.summary || "")}</div></div><div class="row"><button class="btn small" onclick="navigator.clipboard.writeText(briefText());toast('편집 외주서 복사됨')">📋 텍스트 복사</button><button class="btn small" onclick="makeBrief('${esc(p.id)}')">↻ 다시 만들기</button></div></div>
    ...
    <div class="editgrid">${(eb.scenes || []).map(x => `<div class="ecard"><div class="ecard-head"><span class="scene-no">${esc(x.no)}</span><b>${esc(x.sec)}초</b><span class="muted">${esc(x.cut)}</span></div>${kv("자막", esc(x.text))}${kv("전환", esc(x.transition))}${kv("효과음", esc(x.sfx))}${kv("효과", esc(x.effect))}${kv("B롤", esc(x.broll))}${x.note ? `<div class="sb-tip">⚠️ ${esc(x.note)}</div>` : ""}</div>`).join("")}</div>
